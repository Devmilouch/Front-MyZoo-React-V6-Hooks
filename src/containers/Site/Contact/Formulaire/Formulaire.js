...
export default withFormik({
    ...
    validationSchema: Yup.object().shape({
        ...
        message: Yup.string()
        .min(50, "Le message doit avoir au moins 50 caractères")
        .max(200, "Le message doit avoir au maximum 200 caractères")
        .required("Un message est obligatoire !")
    }),
    handleSubmit: (values, { props }) => {
        // alert("Message envoyé !");
        const contenuAEnvoyer = {
            nom: values.nom,
            email: values.email,
            message: values.message
        };
        props.handleEnvoiMail(contenuAEnvoyer);
    }
})(Formulaire);